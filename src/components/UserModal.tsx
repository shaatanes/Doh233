...
export default function UserModal({ user, isClone, upstreams, onClose, onSave }: UserModalProps) {
  ...
  const [dailyTrafficLimitMB, setDailyTrafficLimitMB] = useState(1024);
  ...
  useEffect(() => {
    if (user) {
      ...
      setDailyLimitEnabled(user.dailyLimitEnabled ?? false);
      setDailyTrafficLimitMB(user.dailyTrafficLimit > 0 ? Math.round(user.dailyTrafficLimit / (1024 * 1024)) : 1024);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedUser: User = {
      ...
      unlimitedTraffic,
      unlimitedTime,
      dailyTrafficLimit: dailyLimitEnabled ? dailyTrafficLimitMB * 1024 * 1024 : 0,
      dailyConsumedTraffic: user && !isClone ? user.dailyConsumedTraffic : 0,
      dailyLimitEnabled,
      status,
      allowedUpstreams,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-[#111111] border border-[#262626] rounded max-w-2xl w-full text-slate-100 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            <h3 className="text-[10px] font-bold font-mono text-[#4b5563] uppercase tracking-wider mb-2">اطلاعات کاربری / Client Identity</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-bold font-mono text-[#4b5563] uppercase tracking-wider mb-1">نام کاربری یکتا / Username</label>
                <input
                  type="text"
                  required
                  value={username}
              ...
              <div>
                <label className="block text-[10px] font-bold font-mono text-[#4b5563] uppercase tracking-wider mb-1">نام نمایشی / Alias Name</label>
                <input
                  type="text"
                  required
                  value={displayName}
            ...
            <div className="mt-2">
              <label className="block text-[10px] font-bold font-mono text-[#4b5563] uppercase tracking-wider mb-1">توضیحات دستگاه یا موقعیت / Description</label>
              <input
                type="text"
                value={description}
          ...
          <div>
            <h3 className="text-[10px] font-bold font-mono text-[#4b5563] uppercase tracking-wider mb-2">محدودیت‌ها و زمان انقضا / Limits & Lifespan</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {/* Traffic Limit */}
              <div className="bg-[#0a0a0a] p-3 rounded border border-[#262626]">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-[10px] font-bold font-mono text-[#e5e7eb] uppercase">حجم کل / Total Quota</label>
                  <label className="flex items-center space-x-1 cursor-pointer">
                    ...
                    />
                    <span className="text-[9px] text-[#4b5563] font-mono uppercase">نامحدود</span>
                  </label>
                </div>
                {!unlimitedTraffic ? (
                  <div>
                    <div className="relative">
                      ...
                      />
                      <span className="absolute right-2 top-1.5 text-[9px] text-[#4b5563] font-mono">GB</span>
                    </div>
                    <span className="text-[9px] text-[#4b5563] text-center block mt-1 uppercase font-mono">سقف حجم کل اشتراک</span>
                  </div>
                ) : (
              ...
              <div className="bg-[#0a0a0a] p-3 rounded border border-[#262626]">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-[10px] font-bold font-mono text-[#e5e7eb] uppercase">حجم روزانه / Daily Quota</label>
                  <label className="flex items-center space-x-1 cursor-pointer">
                    <input
                      type="checkbox"
                  ...
                  <div>
                    <div className="relative">
                      <input
                        ...
                        min="1"
                        max="100000"
                        value={dailyTrafficLimitMB}
                        onChange={(e) => setDailyTrafficLimitMB(Number(e.target.value))}
                        className="w-full bg-[#111111] border border-[#262626] rounded px-2.5 py-1 text-xs text-white font-mono text-center focus:border-[#f38020] focus:outline-none"
                      />
                      <span className="absolute right-2 top-1.5 text-[9px] text-[#4b5563] font-mono">MB</span>
                    </div>
                    <span className="text-[9px] text-[#4b5563] text-center block mt-1 uppercase font-mono">سقف مصرف روزانه مجاز</span>
                  </div>
                ) : (
                  <div className="py-1.5 text-center text-[10px] text-[#f38020] font-mono uppercase font-bold tracking-wider">
                    بدون محدودیت روزانه
              ...
              <div className="bg-[#0a0a0a] p-3 rounded border border-[#262626]">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-[10px] font-bold font-mono text-[#e5e7eb] uppercase">تاریخ انقضا / Expiration</label>
                  <label className="flex items-center space-x-1 cursor-pointer">
                    <input
                      type="checkbox"
                      ...
                      className="rounded bg-[#1a1a1a] border-[#262626] text-[#f38020] focus:ring-0 w-3 h-3"
                    />
                    <span className="text-[9px] text-[#4b5563] font-mono uppercase">بدون انقضا</span>
                  </label>
                </div>
                {!unlimitedTime ? (
                  <div>
                    ...
                    />
                    <span className="text-[9px] text-[#4b5563] text-center block mt-1 uppercase font-mono">قطع دسترسی در تاریخ مشخص</span>
                  </div>
                ) : (
                  <div className="py-1.5 text-center text-[10px] text-[#f38020] font-mono uppercase font-bold tracking-wider">
                    اشتراک دائمی
          ...
          {/* Section 3: Status */}
          <div>
            <h3 className="text-[10px] font-bold font-mono text-[#4b5563] uppercase tracking-wider mb-2">وضعیت فعال بودن / Connection Status</h3>
            <div className="bg-[#0a0a0a] p-3 rounded border border-[#262626]">
              ...
              <select
                ...
                className="w-full bg-[#111111] border border-[#262626] rounded px-3 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-[#f38020] font-mono"
              >
                <option value="active">Active (فعال و مجاز)</option>
                <option value="suspended">Suspended (تعلیق موقت)</option>
                <option value="disabled">Disabled (غیرفعال دائم)</option>
              </select>
            </div>
          </div>
          ...
          {/* Collapsible Section 4: Advanced Options */}
          <div>
            <button
              type="button"
              onClick={() => setShowAdvanced(!showAdvanced)}
              className="w-full flex items-center justify-between p-2 bg-[#161616] border border-[#262626] rounded text-[11px] font-bold font-mono text-slate-300 hover:text-white transition-colors"
            >
              <span>تنظیمات پیشرفته / Advanced Policy Rules</span>
              <span className="text-xs">{showAdvanced ? '▲ Collapse' : '▼ Expand'}</span>
            </button>

            {showAdvanced && (
              <div className="mt-3 p-3 bg-[#0a0a0a] border border-[#262626] rounded space-y-4">
                <div>
                      ...
                      <button
                        ...
                        className={`flex items-center space-x-2 p-2 rounded border text-left text-[11px] font-mono transition-colors ${
                          allowedUpstreams.includes(up.id)
                            ...
                            : 'bg-[#111] border-[#262626] text-[#4b5563] hover:border-slate-700'
                        }`}
                      >
                        <input
                          type="checkbox"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#111111] border border-[#262626] rounded px-3 py-1.5 text-xs text-slate-100 font-mono focus:outline-none focus:border-[#f38020]"
                  />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-[#111111] border border-[#262626] rounded px-3 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-[#f38020] resize-none font-mono"
                  />
                </div>
              </div>
            )}
